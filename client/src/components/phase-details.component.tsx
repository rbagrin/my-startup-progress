        ...
        <div>
            <div className='flex flex-row justify-between gap-5 bg-green-100 p-1 rounded-md mb-5'>
                ...
                <div className='p-2 cursor-pointer'>
                    <FaTrash size={15} color='red' onClick={async () => {
            ...
            {phase.tasks.map((task, index) => (
                <div key={index} className='flex flex-row gap-5 mb-5 bg-gray-100 rounded-md justify-between'>
                    ...
                    <div className="flex flex-row gap-1">
                        <div className='p-2 cursor-pointer'>
                            {task.completed ?
                                (<FaRedoAlt size={15} color='#555' onClick={() => markTaskAsIncomplete({ variables: { taskId: task.id } })} />) :
                                (<IoMdCheckmarkCircleOutline size={20} onClick={() => markTaskAsComplete({ variables: { taskId: task.id } })} />)
                            }
                        </div>

                        <div className='p-2 cursor-pointer'>
                            <FaTrash size={15} color='red' onClick={async () => deletePhaseTaskById({ variables: { phaseId: phase.id, taskId: task.id } })} />
                        </div>
                    </div>
                </div>
            ))}
