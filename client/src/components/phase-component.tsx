...
export const PhaseComponent = ({ phase }: PhaseProps) => {
    ...
    return (
        <div onClick={() => navigate(generatePath(PHASE_DETAILS_ROUTE, { phaseId: phase.id }))} className='mb-5'>
            <div className='flex flex-row gap-5 bg-green-100 p-1 rounded-md justify-between'>
                <div className='flex flex-row gap-5'>
                    <div className='mt-2'>
                        {phase.completed ? (<FaCheckCircle size={24} color='green' />) : (<RiCheckboxBlankCircleLine size={25} />)}
                    </div>
