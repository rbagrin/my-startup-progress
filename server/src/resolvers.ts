...
export const resolvers = {
  ...
  Mutation: {
    async markTaskAsComplete(
      ...
      args: { taskId: string },
      { dataSources }: ResolverContext
    ): Promise<TaskAndPhaseMutationResponse> {
      try {
        const { task, phase } = await dataSources.mockDB.markTaskAsComplete(
          args.taskId
        );
    ...
    async markTaskAsIncomplete(
      ...
      try {
        const { task, phase } = await dataSources.mockDB.markTaskAsIncomplete(
          args.taskId
        );
    ...
    async addTaskToPhase(
      _: null,
      args: { phaseId: string; taskDescription: string },
      { dataSources }: ResolverContext
    ): Promise<TaskAndPhaseMutationResponse> {
      try {
        const { task, phase } = await dataSources.mockDB.addTaskToPhase(
          args.phaseId,
    ...
    async addPhase(
      _: null,
      args: { name: string; phaseDescription: string },
      { dataSources }: ResolverContext
    ): Promise<PhasesMutationResponse> {
      try {
        const phases = await dataSources.mockDB.addPhase(
          args.name,
    ...
    async deleteTask(
      _: null,
      args: { phaseId: string; taskId: string },
      { dataSources }: ResolverContext
    ): Promise<PhaseMutationResponse> {
      try {
        const phase = await dataSources.mockDB.deletePhaseTask(
          args.phaseId,
          args.taskId
        );
        return {
